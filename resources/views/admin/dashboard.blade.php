        ...
        <div
            ...
            <div class="flex items-start justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $pendingTopUp ?? 0 }}</p>
                    <p class="text-xs text-gray-400 mt-1">Permintaan baru</p>
                </div>
                <div
            ...
            <a href="{{ route('admin.transactions.index') }}"
                ...
                Proses Sekarang <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div
            ...
            <div class="flex items-start justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $totalUser ?? 0 }}</p>
                    <p class="text-xs text-gray-400 mt-1">Pengguna aktif</p>
                </div>
                <div
                    class="p-2 rounded-md bg-gray-50 text-gray-400 group-hover:bg-black group-hover:text-white transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.users.index') }}"
                class="inline-flex items-center gap-1 text-xs font-semibold text-black hover:underline mt-2">
                Kelola User <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3">
                    </path>
                </svg>
            </a>
        ...
        <div
            ...
            <div class="flex items-start justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $totalAset ?? 0 }}</p>
                    <p class="text-xs text-gray-400 mt-1">Jenis instrumen</p>
                </div>
                <div
                    class="p-2 rounded-md bg-gray-50 text-gray-400 group-hover:bg-black group-hover:text-white transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.assets.index') }}"
                class="inline-flex items-center gap-1 text-xs font-semibold text-black hover:underline mt-2">
                Lihat Aset <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3">
                    </path>
                </svg>
            </a>
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">5 Transaksi Top Up Terakhir</p>
                </div>
                <a href="{{ route('admin.transactions.index') }}"
                    class="text-xs font-bold text-black border-b border-black pb-0.5 hover:opacity-70 transition">
                    Lihat Semua
                </a>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-semibold text-sm text-gray-900">{{ $topup->user->name ?? 'User' }}</p>
                            <p class="text-xs text-gray-500">{{ $topup->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium 
                                {{ $topup->status == 'pending' ? 'bg-gray-200 text-gray-800' : 
                                   ($topup->status == 'approved' ? 'bg-black text-white' : 
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm h-fit">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between p-3 border-b border-gray-100">
                    <span class="text-sm text-gray-500">Versi Aplikasi</span>
                    <span class="text-sm font-bold text-gray-900">v2.1.0 (Stable)</span>
                </div>
                <div class="flex items-center justify-between p-3 border-b border-gray-100">
                    <span class="text-sm text-gray-500">Total Transaksi</span>
                    <span class="text-sm font-bold text-gray-900">{{ $totalTransactions ?? '0' }} Trx</span>
                </div>
                <div class="flex items-center justify-between p-3 border-b border-gray-100">
                    <span class="text-sm text-gray-500">Waktu Server</span>
                    <span class="text-sm font-mono text-gray-900">{{ now()->format('H:i:s') }} UTC+7</span>
                </div>
                <div class="flex items-center justify-between p-3">
                    <span class="text-sm text-gray-500">Status Database</span>
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                        <span class="text-sm font-bold text-gray-900">Connected</span>
                    </div>
                </div>
            </div>
        </div>
